...
<!DOCTYPE html>
<html class="h-full" lang="{{ str_replace('_', '-', app()->getLocale()) }}" @class(['dark'=> ($appearance ?? 'system')
== 'dark'])>
...
<head>
  ...
  <link
    href="https://fonts.bunny.net/css?family=Playfair+Display|IBM+Plex+Sans+Condensed:600|Roboto:900|Open+Sans:400,500,700"
    rel="stylesheet" />
...
<body class="min-h-full flex flex-col font-sans antialiased registration" id="registration">
  ...
  <main class="grow">
    <section class="max-w-4xl mb-10 mt-10 mx-auto {{ $center ? 'text-center' : '' }} px-5 lg:px-0">
      {{ $slot }}
    </section>
  </main>
